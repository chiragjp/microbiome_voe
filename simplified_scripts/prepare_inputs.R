...
main <- function() {
  ...
  final_karlsson <- readRDS('final_Karlsson.rds')
  final_qin <- readRDS('final_Qin.rds')
  final_HMP <- readRDS('final_HMP.rds')
  ...
  browser()
  final_metadata <- merge(final_HMP, karlsson_curated, qin_curated)
  browser()
