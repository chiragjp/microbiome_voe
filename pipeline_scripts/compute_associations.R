...
compute_lm_associations <- function(df, metaphlan_df,phenotype,datatype){
...
#run regression
        ...
        return(tryCatch(tidy(stats::lm(as.formula(str_c("I(log10(`", new_feature_name, "` + ", toString(fudge_factor), ")) ~ .")), # fit lm and get tidy outputs, excluding ID vars (sampleID and subjectID)
                                       data = regression_df))
...
main <- function() {
  ...
  outputname <- args[[3]]
  phenotype=strsplit(args[[1]],'_')[[1]][2]
  datatype=strsplit(args[[1]],'_')[[1]][1]
  output <- compute_lm_associations(cmd_file, datatype_file,phenotype,datatype)
  print(output)
  saveRDS(output, 
          outputname)
}
