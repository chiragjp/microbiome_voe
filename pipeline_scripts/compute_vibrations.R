...
library(rje) # for function powerSet()
...
vibrate <- function(model_df, feature.num, dataset.name, metaphlan_df,pheno) {
  if(pheno=='T2D'){
    ...
    print(str_c("Feature: ", toString(feature.num)))
    new_modeldf <- model_df %>% mutate(dataset_name = dataset.name) 
    feature_num <- as.integer(str_split(feature.num, "_")[[1]][[2]])
    regression_df <- left_join(new_modeldf %>% mutate_if(is.factor, as.character), metaphlan_df %>% select(-dataset_name)%>%select(c(1,2, (feature_num + 1))), by = c("sampleID"))  %>% mutate_if(is.character, as.factor)
    regression_df = regression_df %>% select(-dataset_name,-`1`)
  }
  else{
  fudge_factor <- get_fudge_factor(metaphlan_df) 
  print(str_c("Feature: ", toString(feature.num)))
...
main <- function() {
  ...
  multiplier <- strsplit(args[[3]],'_')[[1]][[3]] #given my naming schema, the three index needs to be changed to a 4 for the gene family data
  multiplier <- as.integer(strsplit(multiplier,'\\.')[[1]][[1]])
  new_features=list()
  features_2=list()
  for(i in seq(length(features))){
    f=as.integer(strsplit(features[[i]],'_')[[1]][[2]])
    f=f-((multiplier-1)*4000)
    if(f<=4000 & f>0){
      new_features[[features[[i]]]]=paste('feature_',as.character(f),sep='')
      features_2[[features[[i]]]]=features[[i]]
    }
  }
  new_features=unname(unlist(new_features))
  features_2=unname(unlist(features_2))
  t_data_type <- as_tibble(readRDS(args[[3]]))
  #t_data_type <- t_data_type[,colnames(t_data_type) %in% features2]
  outputname <- args[[4]]
  pheno <- args[[5]]
  mapping=data.frame(features_2,row.names=new_features,stringsAsFactors = FALSE)
  foo=list.files()
  if(outputname %in% foo == FALSE){
    if(length(new_features)>0){
      print(outputname)
      output <- compute_vibrations(model_dfs, new_features, t_data_type,pheno)
      output$feature <- mapping[output$feature,]
      saveRDS(output,
              outputname)
    }
    else{print('failed, no features to be found here')}
  }
}
